...
int main() {
  ...
  if (HasROCmDevices) {
    std::cout << "Test 'rocm'";
    device d18(filter_selector("rocm"));
    assert(d18.get_platform().get_backend() == backend::rocm);
    std:: cout << "...PASS" << std::endl;

    std::cout << "test 'rocm:gpu'";
    device d19(filter_selector("rocm:gpu"));
    assert(d19.is_gpu() && d19.get_platform().get_backend() == backend::rocm);
    std::cout << "...PASS" << std::endl;
  }
